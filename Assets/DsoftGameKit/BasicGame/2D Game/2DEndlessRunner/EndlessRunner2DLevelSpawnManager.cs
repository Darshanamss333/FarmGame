...
{
    public class EndlessRunner2DLevelSpawnManager : MonoBehaviour
    {
        ...
        private void Update()
        {
            ...
            if(_tang > 1)
            {
                if (Vector3.Distance(_Player.transform.position, GetLast.transform.position) < 10)
                {
                    Spawn();
                }

                if (Vector3.Distance(_Player.transform.position, GetFisrt.transform.position) > 20)
                {
                    DestroyFist();
                }
                _tang = 0;
